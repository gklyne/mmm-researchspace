...
JETTYFOLDER="$SCRIPTPATH/jetty-distribution"
...
# java  -server -Xmx4g -jar "$JETTYFOLDER/start.jar" jetty.base="$JETTYFOLDER" \

java -Xmx4g -jar "$JETTYFOLDER/start.jar" jetty.base="$JETTYFOLDER" \
    --module=logging-jetty  \
    -Dorg.eclipse.jetty.server.Request.maxFormContentSize=100000000 \
    -Djetty.http.port=10214 \
    -Dcom.bigdata.rdf.sail.webapp.ConfigParams.propertyFile="$SCRIPTPATH/RWStore.properties" \
    -Dcom.metaphacts.config.baselocation="$SCRIPTPATH/runtime-data/config" \
    -DruntimeDirectory="$SCRIPTPATH/runtime-data" \
    -DappsDirectory="$SCRIPTPATH/apps" \
    -Dconfig.environment.shiroConfig="$SCRIPTPATH/runtime-data/config/shiro.ini" \
    -Dlog4j.configurationFile="$LOG4J2" \
    -Dconfig.storage.images.type=nonVersionedFile \
    -Dconfig.storage.images.mutable=true \
    -Dconfig.storage.images.root="$SCRIPTPATH/runtime-data/images" \
    -Dconfig.storage.tmp.type=nonVersionedFile \
    -Dconfig.storage.tmp.mutable=true \
    -Dconfig.storage.tmp.root="$SCRIPTPATH/runtime-data/tmp"
